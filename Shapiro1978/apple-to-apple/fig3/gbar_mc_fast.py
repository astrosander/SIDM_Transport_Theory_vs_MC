    ...
    def run_parallel_gbar(n_streams=400, n_relax=6.0, floors=None, clones_per_split=9,
        ...
        # Build ḡ(x) directly from the occupancy histogram.
        # In the Shapiro-Marchant scheme, the MC evolves g(E,J) directly.
        # The occupancy histogram (weighted by time or snapshots) is already
        # proportional to ḡ(x); we just need to divide by bin width and normalize.
        if use_snapshots:
            # Snapshot mode: histogram of weights per bin / (# snapshots * Δx)
            if t0_total <= 0:
                raise RuntimeError("No snapshots taken; check n_relax.")
            g_unscaled = gtime_total / (t0_total * DX)
        else:
            # Time-weighted mode: histogram of w·Δt₀ per bin / (total t₀ * Δx)
            g_unscaled = (gtime_total / t0_total) / DX
        
        # Optional diagnostics: check that occupancy sums to ~1 (for time-weighted mode)
