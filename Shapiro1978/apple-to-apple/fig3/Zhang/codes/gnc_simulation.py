...
class Simulation:
    ...
    def _sample_initial_j(self) -> float:
        ...
        j = max(self.p.jmin, min(j, self.p.jmax))
        return j
    ...
    def compute_gx(self):
        x_bins = np.linspace(self.log10_emin, np.log10(self.p.x_boundary), self.p.gx_bins)
        x_centers = 0.5 * (x_bins[:-1] + x_bins[1:])
        
        if len(x_centers) < self.p.gx_bins:
    ...
    def run_snapshot(self, t_start: float, t_end: float):
        ...
        for p in self.particles:
            ...
            a = self.p.mbh / (-2 * p.energy)
            period = 2 * np.pi * np.sqrt(a**3 / self.p.mbh) / period_factor
